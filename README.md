...
| 탭 | 설명 |
|----|------|
| **카메라** | 후면 카메라 실제 화면 위에 지점 핀·화면밖 화살표·나침반 밴드·크로스헤어를 오버레이. 나침반은 저역통과 보간으로 부드럽게 회전(깜박임 없음). 셔터로 AR 합성 사진 촬영. |
| **지도** | Leaflet(OSM) 지도 + VWorld 토지이용계획도 오버레이(투명도 조절). 지도를 누르면 **이름·그룹을 바로 지정**하는 시트로 지점 추가, 마커를 눌러 편집. 최초 1회만 내 위치로 이동(이후 자동 이동 없음, ‘내 위치로’ 버튼은 수동). |
| **지점** | 서버에 **공유 저장**된 지점을 **그룹별**로, 내 위치 기준 거리·방위와 함께 목록으로. 그룹 추가/이름변경/색변경/삭제, 지점 편집·삭제. 표시숨김(눈)만 기기별 로컬. |
...
**지점·그룹은 서버에 공유 저장**됩니다(아래 [지점 공유 저장소](#지점-공유-저장소-백엔드-api) 참고).
링크(`arcam.3chan.kr`)를 여는 **모든 기기가 같은 지점 목록**을 보고, 한 기기에서 추가·수정·삭제하면
...
지점·그룹을 모든 기기가 공유하도록 **의존성 없는 작은 Node 백엔드**(`backend/server.js`)를 둡니다.

- **저장**: 지점·그룹을 원자적 JSON 파일 하나(`data/store.json`, 바인드 마운트로 재시작·재배포에도 유지)에
...
- **사진 API**: 사진은 지점 store와 **완전히 분리** 저장합니다 — 원본/썸네일 JPEG은 파일로(`data/photos/<id>.jpg`,
  `<id>.t.jpg`), 메타 색인만 `data/photos.json`에. 그래서 잦은 지점 저장이 대용량 이미지로 느려지지 않습니다.
  ...
  재접속 시 재시도합니다(사진 유실 방지). 사진도 지점처럼 **모든 기기 공유 + 15초 폴링**입니다.
- **접근 제어 없음(공개)** — 검증(위경도 범위·이름 길이·색상·JPEG 매직바이트)과 상한(지점 1만·그룹 500개·본문
...
- **클라이언트 동기화**: 서버가 원본. 부팅 시 서버에서 받아오고, 보이는 동안 15초마다 + 탭 복귀·온라인 복귀 시
  ...
  로컬 지점은 서버가 비어 있을 때 **최초 1회 자동 이관**됩니다.
...
├─ backend/
│  └─ server.js             # 지점·그룹 공유 저장소 API (의존성 없는 Node HTTP 서버)
